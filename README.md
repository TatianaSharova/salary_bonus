...
Если направлений больше 20, баллы вычисляются по формуле: (количество направлений/(8.5-сложность) + сложность/2)
...
**_Объект культурным наследия:_**
Проверяем ябляется ли объект культурным наслединем (check_cultural_heritage):
...
Все функции для расспределения баллов по кварталам находятся в файле quaterly_points.py.
...
Все функции для подключения к таблицам, форматирования, отпрвлениия данных находятся в файле worksheeys.py
...
Для каждого проектировщика бот создает собственный лист, куда отправляются данные о его проектах. Если для каких-то проектировщиков не надо делать расчет баллов, их фамилии через запятую можно перечислить в листе "Настройки" (этот лист создается автоматически) в таблице "Не учитывать". Пример:
...
Например, [здесь рассказывают, как получить этот ключ.](https://codd-wd.ru/instrukciya-po-polucheniyu-klyucha-servisnogo-akkaunta-google-dlya-raboty-s-sheets-api/ "Гиперссылка")_** (на 3 шаге не забыть добавить доступ к Google Sheets API)
...
Вы можете либо сами создать талицу с названием "Премирование2024" и дать к ней доступ боту, либо в функции worksheets/connect_to_bonus_ws() указать свою почту, чтобы бот пошерил с вами доступ к новой созданной таблице 
...
## План по доработке:
1. Доработать функцию по вычислению сложности проекта (более детально).
2. Сделать так, чтобы ГИП мог редактировать сложность проекта, если считает ее неправильно определенной.
3. Настроить автоматическое обновление пакета holidays внутри контейнера на сервере.
4. Считать дедлайн для блок-контейнеров (для них ставят общую дату начала и сдачи)
5. К какому периоду времени привязывать корректировки.
6. Вывести на листы проектировщиков план выполнения проектов в баллах (на данный момент это 12) и какое получается перевыполнение.
