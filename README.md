...
Готово, при пуше в ветку main проект должен развернуться на сервере. Программа сразу после загрузки на сервер сделает расчет баллов, потом будет делать расчет каждый день в 10:10 утра. После окончания деплоя и каждый раз после выполнения расчета тг-бот будет слать уведомление об этом.
