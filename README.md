...
Готово, при пуше в ветку main проект должен развернуться на сервере. Программа сразу после загрузки на сервер сделает расчет баллов, потом будет делать расчет по вторникам и пятницам в 10:00 утра. Также каждого 1 декабря в 9:00 будет обновляться пакет holidays для корректного подсчета дедлайна.
