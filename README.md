...
Если сложность расчитана некорректно, ГИП может её поменять в таблице "Премирование". Для этого в столбце "Корректировка сложности" нужно записать новое значение сложности. При следующем расчете баллы за проект будут считаться, исходя из сложности, поставленной ГИПом. Ячейки с учтенными корректировками окрашиваются в желтый цвет, чтобы можно было определить, стоят ли новые баллы за проект или старые. 
...
Для блок-контейнеров баллы расчитываются немного иначе.                                                      
Если была выполнена группа блок-контейнеров (у которых совпадает название в столбце "Наименование объекта"), то первый разработанный блок-контейнер получает полное количество посчитанных баллов. Остальным блок-контейнерам с таким же названием дается по половине баллов от баллов первого контейнера.
Для группы блок-контейнеров дедлайн расчитывается по формуле: 4 + (количество блок-контейнеров).                                     
...
Чтобы учесть, что проект является корректировкой, необходимо в стоблце "Является корректировкой" поставить значение "Да".
